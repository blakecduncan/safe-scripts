...
import { SAFE, SAFE_FACTORY, SAFE_ENTRY_POINT, EIP_4337_MANAGER } from "../constants";
...
export async function getContracts() {
  ...
  const accountFactory = await getSafeProxyFactory(proxyFactory.address, safeSingleton.address, manager.address)
...
function getSafeProxyFactory(proxyFactory: string, safeSingleton: string, manager: string) {
  const safeProxyFactory__Factory = new ContractFactory(SafeAccountFactory.abi, SafeAccountFactory.bytecode, getSigner())
