...
async function sendEth() {
  ...
  const op = await fillAndSign(
    ...
    await getOwner(ownerPk),
    entryPoint,
  );

  const bundlerUrl = 'http://127.0.0.1:3000/rpc';
  const entryPointAddress = '0x24DcF7Fa50F0Ec9d8972Bf0aB1FF5361B796E6BF';
  ...
  const userOpHash = await httpRpcClient.sendUserOpToBundler(op);
  console.log('userOpHash=', userOpHash);
  console.log('-----------');

  //code to wait two seconds
  await new Promise((resolve) => setTimeout(resolve, 2000));

  const safeBalanceAfter = await provider.getBalance(safeAddress);
  console.log(
    `Safe balance Before : ${ethers.utils.formatEther(safeBalanceBefore)} ETH`,
  );
  console.log(
    `Safe balance After : ${ethers.utils.formatEther(safeBalanceAfter)} ETH`,
  );
  console.log('-----------');

  const receiveAddress = await provider.getBalance(to);
  console.log(
    `To balance After : ${ethers.utils.formatEther(receiveAddress)} ETH`,
  );
}
