...
import { Wallet, Signer, ethers, BigNumber, Contract } from 'ethers'
...
export async function fillUserOp (op: Partial<UserOperation>, entryPoint?: any, getNonceFunction = 'getNonce'): Promise<UserOperation> {
  const op1 = { ...op }
  ...
  if (op.initCode != null) {
    ...
    if (op1.sender == null) {
      ...
      if (initAddr.toLowerCase() === Create2Factory.contractAddress.toLowerCase()) {
        ...
        op1.sender = await entryPoint!.callStatic.getSenderAddress(op1.initCode!).catch((e: { errorArgs: { sender: any } }) => e.errorArgs.sender)
      }
    }
...
export async function fillAndSign (op: Partial<UserOperation>, signer: Wallet | Signer, entryPoint?: any, getNonceFunction = 'getNonce'): Promise<UserOperation> {
  const provider = entryPoint?.provider
  const op2 = await fillUserOp(op, entryPoint, getNonceFunction)

  const chainId = await provider!.getNetwork().then((net: { chainId: string }) => net.chainId)
